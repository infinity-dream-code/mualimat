...
@section('script')
    ...
    <script>
        ...
        document.getElementById('cetak-kartu-siswa').addEventListener('click', function (e) {
            ...
            let url = '{{route('admin.rekap-cek-pelunasan.cetak-kartu-siswa')}}';
            ...
            let data = window.DT[`${dtOptions.tableId}`].rows({selected: true}).data();

            if (!data || !data[0]) {
                warningAlert('Silahkan pilih siswa terlebih dahulu!');
                Swal.close();
                return;
            }
