...
class PotonganTagihanController extends Controller
{
    ...
    public function create()
    {
        ...
        $data["columnsUrl"] = route("admin.potongan-tagihan.create.get-column");
        $data["datasUrl"] = route("admin.potongan-tagihan.create.get-data");
        $data["post"] = mst_tagihan::select(["tagihan"])
            ->orderByRaw(
                "
                        CASE
    ...
    public function getColumnCreate()
    {
        return [
            [
                "data" => "AA",
    ...
    public function getDataCreate(Request $request)
    {
        ...
        $baseQuery = function () use ($whereAny, $searchValue, $filterQuery) {
            return scctbill::query()
                ...
                ->where("scctbill.PAIDST", 1)
                ->where("scctbill.FSTSBolehBayar", 1)
                ->whereDoesntHave('cuts', function ($query) {
        ...
        $totalRecords = scctbill::where("PAIDST", 1)
